...
class BitcostReadinessConfig:
    ...
    def normalized(self) -> "BitcostReadinessConfig":
        ...
        self.diversity_activation_mode = str(
            self.diversity_activation_mode
        ).lower().strip()
        self.diversity_min_sample_stride_seconds = float(
            self.diversity_min_sample_stride_seconds
        )
        ...
        if self.diversity_min_sample_stride_seconds < 0.0:
            raise ValueError(

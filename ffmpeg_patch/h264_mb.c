                                                       ...
                                                       uint8_t *dest_y, int p)
{
    ...
    // Pre-fill macroblock with 128 for residual visualization mode.
    // This ensures that when nnz==0 (no residual), the buffer contains 128
    // instead of the prediction value.
    if (!pixel_shift) {
        for (i = 0; i < 16; i++)
            memset(dest_y + block_offset[i], 128, 4);
    } else {
        for (i = 0; i < 16; i++) {
            uint16_t *row = (uint16_t *)(dest_y + block_offset[i]);
            for (int j = 0; j < 4; j++)
                row[j] = 128 << (pixel_shift - 1);
        }
    }
            ...
            for (i = 0; i < 16; i += 4) {
                ...
                if (transform_bypass && h->ps.sps->profile_idc == 244 && dir <= 1) {
                    ...
                    if (nnz) {
                        ...
                        // For residual visualization: fill with 128 when no residual
                        if (!pixel_shift) {
                            for (int yy = 0; yy < 8; yy++)
                                memset(ptr + yy * linesize, 128, 8);
                        } else {
                            for (int yy = 0; yy < 8; yy++) {
                                uint16_t *row = (uint16_t *)(ptr + yy * linesize);
                                for (int xx = 0; xx < 8; xx++)
                                    row[xx] = 128 << (pixel_shift - 1);
                            }
                        }
                    }
                }

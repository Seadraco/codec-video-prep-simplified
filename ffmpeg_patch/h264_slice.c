...
{
    const int mv_only = !!(h->avctx->debug & 0x40000000);
...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    const H264Context *h = sl->h264;
    const int bitcost_only = avctx->skip_idct == AVDISCARD_ALL;
    const int mv_only = !!(avctx->debug & 0x40000000);
    const int skip_recon = bitcost_only || mv_only;
        ...
        for (;;) {
            ...
            if ((h->workaround_bugs & FF_BUG_TRUNCATED) &&
                ...
                er_add_slice(sl, sl->resync_mb_x, sl->resync_mb_y, sl->mb_x - 1,
                             sl->mb_y, ER_MB_END);
                if (!skip_recon && sl->mb_x >= lf_x_start)

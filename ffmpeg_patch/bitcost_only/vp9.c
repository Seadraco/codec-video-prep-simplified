...
#define VP9_SB_BIT_COST_MAP_MAGIC  MKBETAG('V', 'P', '9', 'S')
...
static int vp9_bitcost_enabled_for_current_frame(AVCodecContext *avctx)
{
    const VP9Context *s = avctx->priv_data;
    const CvrTargetBitcostCtx *ctx;
    int frame_number, tol, i;

    if (!s || !avctx || !avctx->opaque)
        return 1;

    ctx = (const CvrTargetBitcostCtx *)avctx->opaque;
...
static int vp9_rac_pos_bits(const VP56RangeCoder *c, const uint8_t *start)
{
    return (int)((c->buffer - start) * 8 + c->bits - 8);
}

static int32_t *vp9_sb_costs_ptr(Vp9SbBitCostMap *map)
{
    return map->costs;
}

static float *vp9_sub_costs_ptr(Vp9SbBitCostMap *map)
{
    return (float *)(map->costs + (size_t)map->sb_stride * map->sb_height);
}

static Vp9SbBitCostMap *vp9_get_sb_bit_cost_map(AVCodecContext *avctx)
{
    VP9Context *s = avctx->priv_data;
    AVBufferRef *buf;
    Vp9SbBitCostMap *map;
    size_t size;
    int sb_width = (s->w + 63) / 64;
    int sb_height = (s->h + 63) / 64;
    int sb_stride = sb_width;

    if (!vp9_bitcost_enabled_for_current_frame(avctx))
        return NULL;

    size = sizeof(Vp9SbBitCostMap) +
           (size_t)sb_stride * sb_height * sizeof(int32_t) +
           (size_t)(sb_stride * 16) * (sb_height * 16) * sizeof(float);

    buf = s->s.frames[CUR_FRAME].bitcost_buf;

    if (!buf || buf->size != size) {
        av_buffer_unref(&s->s.frames[CUR_FRAME].bitcost_buf);
        buf = av_buffer_allocz(size);
        if (!buf)
            return NULL;
        s->s.frames[CUR_FRAME].bitcost_buf = buf;
    }

    map = (Vp9SbBitCostMap *)buf->data;
    if (map->magic != VP9_SB_BIT_COST_MAP_MAGIC ||
        map->version != VP9_SB_BIT_COST_MAP_VERSION ||
        map->sb_width != sb_width ||
        map->sb_height != sb_height ||
        map->sb_stride != sb_stride) {
        memset(buf->data, 0, size);
        map->magic = VP9_SB_BIT_COST_MAP_MAGIC;
        map->version = VP9_SB_BIT_COST_MAP_VERSION;
        map->sb_width = sb_width;
        map->sb_height = sb_height;
        map->sb_stride = sb_stride;
        map->sub_width = sb_width * 16;
        map->sub_height = sb_height * 16;
...
static void vp9_store_sb_bit_cost(AVCodecContext *avctx, VP9Context *s, int sb_col, int sb_row, int cost_bits)
{
    Vp9SbBitCostMap *map = vp9_get_sb_bit_cost_map(avctx);
    int sb_idx;
    float *sub;
    int sub_w, sub_h, x, y;
    ...
    sb_idx = sb_row * map->sb_stride + sb_col;
    if (sb_idx < 0 || sb_idx >= map->sb_stride * map->sb_height)
        return;

    vp9_sb_costs_ptr(map)[sb_idx] = cost_bits;

    sub_w = FFMIN(16, map->sub_width - sb_col * 16);
    sub_h = FFMIN(16, map->sub_height - sb_row * 16);
    sub = vp9_sub_costs_ptr(map);
    for (y = sb_row * 16; y < sb_row * 16 + sub_h; y++) {
        for (x = sb_col * 16; x < sb_col * 16 + sub_w; x++) {
            sub[y * map->sub_stride + x] = (float)cost_bits / (sub_w * sub_h);
        }
    }
}
        ...
        for (row = tile_row_start; row < tile_row_end;
            ...
            for (tile_col = 0; tile_col < s->s.h.tiling.tile_cols; tile_col++) {
                ...
                if (s->pass != 2) {
                    ...
                    td->rc_start = td->c_b[tile_col].buffer - 3;
                }
                ...
                for (col = tile_col_start;
                    ...
                    if (s->pass == 2) {
                        ...
                        int bits_before = 0, bits_after = 0;
                        if (vp9_bitcost_enabled_for_current_frame(avctx))
                            bits_before = vp9_rac_pos_bits(td->c, td->rc_start);
                        ...
                        decode_sb(td, row, col, lflvl_ptr,
                                  yoff2, uvoff2, BL_64X64);
                        if (vp9_bitcost_enabled_for_current_frame(avctx)) {
                            bits_after = vp9_rac_pos_bits(td->c, td->rc_start);
                            vp9_store_sb_bit_cost(avctx, s, col >> 3, row >> 3,
                              ...
                              int threadnr)
{
    ...
    for (tile_row = 0; tile_row < s->s.h.tiling.tile_rows; tile_row++) {
        ...
        td->rc_start = td->c_b[tile_row].buffer - 3;
        for (row = tile_row_start; row < tile_row_end;
             row += 8, yoff += ls_y * 64, uvoff += ls_uv * 64 >> s->ss_v) {
            ptrdiff_t yoff2 = yoff, uvoff2 = uvoff;
...
finish:
    ...
    if (s->s.frames[CUR_FRAME].bitcost_buf) {
        ret = av_buffer_replace(&s->s.frames[CUR_FRAME].tf.f->opaque_ref,
                                s->s.frames[CUR_FRAME].bitcost_buf);
        if (ret < 0)
            return ret;
    }

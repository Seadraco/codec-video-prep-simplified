...
void ff_hevc_unref_frame(HEVCContext *s, HEVCFrame *frame, int flags)
{
    ...
    frame->flags &= ~flags;
    if (!frame->flags) {
        av_buffer_unref(&frame->frame_grain->opaque_ref);
        av_buffer_unref(&frame->ctu_bit_cost_buf);
...
int ff_hevc_set_new_ref(HEVCContext *s, AVFrame **frame, int poc)
{
    ...
    av_buffer_unref(&ref->frame_grain->opaque_ref);
    av_buffer_unref(&ref->ctu_bit_cost_buf);
...
int ff_hevc_output_frame(HEVCContext *s, AVFrame *out, int flush)
{
    do {
        ...
        if (nb_output) {
            ...
            /* output_frame_poc logic removed */

            if (frame->frame->opaque_ref) {
                opaque_ref = av_buffer_ref(frame->frame->opaque_ref);
                if (!opaque_ref)
                    return AVERROR(ENOMEM);
            } else if (frame->ctu_bit_cost_buf) {
                opaque_ref = av_buffer_ref(frame->ctu_bit_cost_buf);
                if (!opaque_ref)
                    return AVERROR(ENOMEM);
            }
            ...
            if (opaque_ref) {
                av_buffer_unref(&out->opaque_ref);
                out->opaque_ref = opaque_ref;
            }

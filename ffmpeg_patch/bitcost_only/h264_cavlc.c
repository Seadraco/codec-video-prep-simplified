...
#define H264_MB_BIT_COST_MAP_MAGIC MKBETAG('M', 'B', 'C', 'T')
...
#define CVR_TARGET_BITCOST_MAGIC MKBETAG('C', 'V', 'R', 'T')
...
static int h264_bitcost_enabled_for_current_frame(const H264Context *h)
{
    const CvrTargetBitcostCtx *ctx;
    int frame_number, tol, i;

    if (!h || !h->avctx || !h->avctx->opaque)
        return 1;

    ctx = (const CvrTargetBitcostCtx *)h->avctx->opaque;
    if (ctx->magic != CVR_TARGET_BITCOST_MAGIC)
        return 1;
    if (!ctx->enabled)
        return 1;
    if (!ctx->frame_bitmap || ctx->max_frame < 0)
        return 0;

    frame_number = h->avctx->frame_number;
    tol = ctx->tolerance < 0 ? 0 : ctx->tolerance;
    for (i = -tol - 1; i <= tol + 1; ++i) {
        int idx = frame_number + i;
        if (idx >= 0 && idx <= ctx->max_frame && ctx->frame_bitmap[idx])
            return 1;
    }
    return 0;
}

static int32_t *h264_mb_costs_ptr(H264MbBitCostMap *map)
{
    return map->costs;
}

static float *h264_sub_costs_ptr(H264MbBitCostMap *map)
{
    return (float *)(map->costs + (size_t)map->mb_stride * map->mb_height);
}

static H264MbBitCostMap *h264_get_mb_bit_cost_map(const H264Context *h)
{
    AVBufferRef *buf;
    H264MbBitCostMap *map;
    size_t size;

    if (!h || !h->cur_pic_ptr || !h->cur_pic_ptr->f)
        return NULL;
    if (!h264_bitcost_enabled_for_current_frame(h))
        return NULL;

    size = sizeof(*map) +
           (size_t)h->mb_stride * h->mb_height * sizeof(int32_t) +
           (size_t)(h->mb_width * 4) * (h->mb_height * 4) * sizeof(float);
    buf = h->cur_pic_ptr->bitcost_buf;

    if (!buf || buf->size != size) {
        av_buffer_unref(&h->cur_pic_ptr->bitcost_buf);
        buf = av_buffer_allocz(size);
        if (!buf)
            return NULL;
        h->cur_pic_ptr->bitcost_buf = buf;
    }

    map = (H264MbBitCostMap *)buf->data;
    if (map->magic != H264_MB_BIT_COST_MAP_MAGIC ||
        map->version != H264_MB_BIT_COST_MAP_VERSION ||
        map->mb_width != h->mb_width ||
        map->mb_height != h->mb_height ||
        map->mb_stride != h->mb_stride) {
        memset(buf->data, 0, size);
        map = (H264MbBitCostMap *)buf->data;
        map->magic     = H264_MB_BIT_COST_MAP_MAGIC;
        map->version   = H264_MB_BIT_COST_MAP_VERSION;
        map->mb_width  = h->mb_width;
        map->mb_height = h->mb_height;
        map->mb_stride = h->mb_stride;
        map->sub_width = h->mb_width * 4;
        map->sub_height = h->mb_height * 4;
        map->sub_stride = map->sub_width;
    }

    return map;
}

static void h264_store_mb_bit_cost(const H264Context *h, int mb_xy, int cost_bits)
{
    H264MbBitCostMap *map = h264_get_mb_bit_cost_map(h);
    if (!map || mb_xy < 0 || mb_xy >= map->mb_stride * map->mb_height)
        return;
    if (mb_xy == 0) {
        memset(h264_mb_costs_ptr(map), 0, (size_t)map->mb_stride * map->mb_height * sizeof(int32_t));
        memset(h264_sub_costs_ptr(map), 0, (size_t)map->sub_stride * map->sub_height * sizeof(float));
    }
    h264_mb_costs_ptr(map)[mb_xy] = cost_bits;
}

static void h264_add_sub_mb_bit_cost_rect(const H264Context *h,
                                          int mb_xy,
                                          int x4_off, int y4_off,
                                          int w4, int h4,
                                          float bits)
{
    H264MbBitCostMap *map = h264_get_mb_bit_cost_map(h);
    float *sub_costs;
    int mb_x, mb_y, gx0, gy0, gx1, gy1, x, y;
    float per_cell;

    if (!map || bits == 0.0f || w4 <= 0 || h4 <= 0)
        return;

    mb_x = mb_xy % map->mb_stride;
    mb_y = mb_xy / map->mb_stride;
    gx0 = mb_x * 4 + x4_off;
    gy0 = mb_y * 4 + y4_off;
    gx1 = gx0 + w4;
    gy1 = gy0 + h4;
    if (gx0 < 0 || gy0 < 0 || gx1 > map->sub_width || gy1 > map->sub_height)
        return;

    per_cell = bits / (float)(w4 * h4);
    sub_costs = h264_sub_costs_ptr(map);
    for (y = gy0; y < gy1; ++y) {
        float *row = sub_costs + (size_t)y * map->sub_stride;
        for (x = gx0; x < gx1; ++x)
            row[x] += per_cell;
    }
}

static void h264_add_sub_mb_bit_cost_full_mb(const H264Context *h, int mb_xy, float bits)
{
    h264_add_sub_mb_bit_cost_rect(h, mb_xy, 0, 0, 4, 4, bits);
}

static void h264_sub_mb_partition_rect(uint32_t sub_mb_type, int part_idx,
                                       int *x4, int *y4, int *w4, int *h4)
{
    if (IS_SUB_8X8(sub_mb_type)) {
        *x4 = 0; *y4 = 0; *w4 = 2; *h4 = 2;
    } else if (IS_SUB_8X4(sub_mb_type)) {
        *x4 = 0; *y4 = part_idx; *w4 = 2; *h4 = 1;
    } else if (IS_SUB_4X8(sub_mb_type)) {
        *x4 = part_idx; *y4 = 0; *w4 = 1; *h4 = 2;
    } else {
        *x4 = part_idx & 1;
        *y4 = part_idx >> 1;
        *w4 = 1;
...
int ff_h264_decode_mb_cavlc(const H264Context *h, H264SliceContext *sl)
{
    ...
    bits_before_mb = get_bits_count(&sl->gb);
    ...
    if (sl->slice_type_nos != AV_PICTURE_TYPE_I) {
        if (sl->mb_skip_run == -1) {
            const int bits_before_skip_run = get_bits_count(&sl->gb);
            ...
            skip_run_cost = get_bits_count(&sl->gb) - bits_before_skip_run;
        }
        ...
        if (sl->mb_skip_run--) {
            ...
            decode_mb_skip(h, sl);
            h264_store_mb_bit_cost(h, mb_xy, skip_run_cost);
            h264_add_sub_mb_bit_cost_full_mb(h, mb_xy, (float)skip_run_cost);
            return 0;
        }
    }
    if (FRAME_MBAFF(h)) {
...
decode_intra_mb:
    ...
    if(IS_INTRA_PCM(mb_type)){
        ...
        {
            const int total_bits = get_bits_count(&sl->gb) - bits_before_mb;
            h264_store_mb_bit_cost(h, mb_xy, total_bits);
            h264_add_sub_mb_bit_cost_full_mb(h, mb_xy, (float)total_bits);
        }
        return 0;
    }
...
//            init_top_left_availability(h);
        if(IS_INTRA4x4(mb_type)){
            int i;
            int di = 1;
            int bits_before_flag = get_bits_count(&sl->gb);
            if(dct8x8_allowed && get_bits1(&sl->gb)){
                mb_type |= MB_TYPE_8x8DCT;
                di = 4;
            }
            if (get_bits_count(&sl->gb) > bits_before_flag) {
                const int flag_bits = get_bits_count(&sl->gb) - bits_before_flag;
                h264_add_sub_mb_bit_cost_full_mb(h, mb_xy, (float)flag_bits);
                sub_accounted_bits += (float)flag_bits;
            }

//                fill_intra4x4_pred_table(h);
            for(i=0; i<16; i+=di){
                int bits_before_pred = get_bits_count(&sl->gb);
                ...
                if (get_bits_count(&sl->gb) > bits_before_pred) {
                    const int pred_bits = get_bits_count(&sl->gb) - bits_before_pred;
                    const int x4 = i & 3;
                    const int y4 = i >> 2;
                    h264_add_sub_mb_bit_cost_rect(h, mb_xy, x4, y4, di, di, (float)pred_bits);
                    sub_accounted_bits += (float)pred_bits;
                }
        ...
        if(decode_chroma){
            int bits_before_chroma_pred = get_bits_count(&sl->gb);
            ...
            if (get_bits_count(&sl->gb) > bits_before_chroma_pred) {
                const int chroma_bits = get_bits_count(&sl->gb) - bits_before_chroma_pred;
                h264_add_sub_mb_bit_cost_full_mb(h, mb_xy, (float)chroma_bits);
                sub_accounted_bits += (float)chroma_bits;
            }
        } else {
        ...
        if (sl->slice_type_nos == AV_PICTURE_TYPE_B) {
            for(i=0; i<4; i++){
                int bits_before_sub_type = get_bits_count(&sl->gb);
                sl->sub_mb_type[i]= get_ue_golomb_31(&sl->gb);
                if(sl->sub_mb_type[i] >=13){
                    av_log(h->avctx, AV_LOG_ERROR, "B sub_mb_type %u out of range at %d %d\n", sl->sub_mb_type[i], sl->mb_x, sl->mb_y);
                    return -1;
                }
                if (get_bits_count(&sl->gb) > bits_before_sub_type) {
                    const int sub_type_bits = get_bits_count(&sl->gb) - bits_before_sub_type;
                    h264_add_sub_mb_bit_cost_rect(h, mb_xy, (i & 1) * 2, (i >> 1) * 2, 2, 2, (float)sub_type_bits);
                    sub_accounted_bits += (float)sub_type_bits;
                }
        ...
        for (list = 0; list < sl->list_count; list++) {
            ...
            for(i=0; i<4; i++){
                ...
                if(IS_DIR(sl->sub_mb_type[i], 0, list)){
                    unsigned int tmp;
                    int bits_before_ref = get_bits_count(&sl->gb);
                    if(ref_count == 1){
                        tmp= 0;
                    }else if(ref_count == 2){
                    ...
                    ref[list][i]= tmp;
                    if (get_bits_count(&sl->gb) > bits_before_ref) {
                        const int ref_bits = get_bits_count(&sl->gb) - bits_before_ref;
                        h264_add_sub_mb_bit_cost_rect(h, mb_xy, (i & 1) * 2, (i >> 1) * 2, 2, 2, (float)ref_bits);
                        sub_accounted_bits += (float)ref_bits;
                    }
                }else{
                 //FIXME
                    ref[list][i] = -1;
            ...
            for(i=0; i<4; i++){
                ...
                if(IS_DIR(sl->sub_mb_type[i], 0, list)){
                    ...
                    for(j=0; j<sub_partition_count[i]; j++){
                        ...
                        int bits_before_mvd = get_bits_count(&sl->gb);
                        pred_motion(h, sl, index, block_width, list, sl->ref_cache[list][ scan8[index] ], &mx, &my);
                        mx += (unsigned)get_se_golomb(&sl->gb);
                        my += (unsigned)get_se_golomb(&sl->gb);
                        if (get_bits_count(&sl->gb) > bits_before_mvd) {
                            int lx4, ly4, lw4, lh4;
                            const int mvd_bits = get_bits_count(&sl->gb) - bits_before_mvd;
                            h264_sub_mb_partition_rect(sub_mb_type, j, &lx4, &ly4, &lw4, &lh4);
                            h264_add_sub_mb_bit_cost_rect(h, mb_xy, (i & 1) * 2 + lx4, (i >> 1) * 2 + ly4,
                                                          lw4, lh4, (float)mvd_bits);
                            sub_accounted_bits += (float)mvd_bits;
                        }
        ...
        if(IS_16X16(mb_type)){
            for (list = 0; list < sl->list_count; list++) {
                    ...
                    fill_rectangle(&sl->ref_cache[list][ scan8[0] ], 4, 4, 8, val, 1);
                        if (get_bits_count(&sl->gb) > bits_before_ref) {
                            const int ref_bits = get_bits_count(&sl->gb) - bits_before_ref;
                            h264_add_sub_mb_bit_cost_full_mb(h, mb_xy, (float)ref_bits);
                            sub_accounted_bits += (float)ref_bits;
                        }
                    }
            }
            for (list = 0; list < sl->list_count; list++) {
                if(IS_DIR(mb_type, 0, list)){
                    ...
                    if (get_bits_count(&sl->gb) > bits_before_mvd) {
                        const int mvd_bits = get_bits_count(&sl->gb) - bits_before_mvd;
                        h264_add_sub_mb_bit_cost_full_mb(h, mb_xy, (float)mvd_bits);
                        sub_accounted_bits += (float)mvd_bits;
                    }
                    ff_tlog(h->avctx, "final mv:%d %d\n", mx, my);
            ...
            for (list = 0; list < sl->list_count; list++) {
                    for(i=0; i<2; i++){
                        unsigned int val;
                        int bits_before_ref = 0;
                        if(IS_DIR(mb_type, i, list)){
                            unsigned rc = sl->ref_count[list] << MB_MBAFF(sl);
                            bits_before_ref = get_bits_count(&sl->gb);
                            if (rc == 1) {
                                val= 0;
                            } else if (rc == 2) {
                                val= get_bits1(&sl->gb)^1;
                        ...
                        if (IS_DIR(mb_type, i, list) && get_bits_count(&sl->gb) > bits_before_ref) {
                            const int ref_bits = get_bits_count(&sl->gb) - bits_before_ref;
                            h264_add_sub_mb_bit_cost_rect(h, mb_xy, 0, 2 * i, 4, 2, (float)ref_bits);
                            sub_accounted_bits += (float)ref_bits;
                        }
                    }
            }
            for (list = 0; list < sl->list_count; list++) {
                for(i=0; i<2; i++){
                    ...
                    if(IS_DIR(mb_type, i, list)){
                        ...
                        if (get_bits_count(&sl->gb) > bits_before_mvd) {
                            const int mvd_bits = get_bits_count(&sl->gb) - bits_before_mvd;
                            h264_add_sub_mb_bit_cost_rect(h, mb_xy, 0, 2 * i, 4, 2, (float)mvd_bits);
                            sub_accounted_bits += (float)mvd_bits;
                        }
                        ff_tlog(h->avctx, "final mv:%d %d\n", mx, my);
            ...
            for (list = 0; list < sl->list_count; list++) {
                    for(i=0; i<2; i++){
                        ...
                        if (IS_DIR(mb_type, i, list) && get_bits_count(&sl->gb) > bits_before_ref) {
                            const int ref_bits = get_bits_count(&sl->gb) - bits_before_ref;
                            h264_add_sub_mb_bit_cost_rect(h, mb_xy, 2 * i, 0, 2, 4, (float)ref_bits);
                            sub_accounted_bits += (float)ref_bits;
                        }
                    }
            }
            for (list = 0; list < sl->list_count; list++) {
                for(i=0; i<2; i++){
                    ...
                    if(IS_DIR(mb_type, i, list)){
                        ...
                        if (get_bits_count(&sl->gb) > bits_before_mvd) {
                            const int mvd_bits = get_bits_count(&sl->gb) - bits_before_mvd;
                            h264_add_sub_mb_bit_cost_rect(h, mb_xy, 2 * i, 0, 2, 4, (float)mvd_bits);
                            sub_accounted_bits += (float)mvd_bits;
                        }
                        ff_tlog(h->avctx, "final mv:%d %d\n", mx, my);
    ...
    {
        const int total_bits = get_bits_count(&sl->gb) - bits_before_mb;
        const float remaining = (float)total_bits - sub_accounted_bits;
        h264_store_mb_bit_cost(h, mb_xy, total_bits);
        if (remaining > 0.0f)
            h264_add_sub_mb_bit_cost_full_mb(h, mb_xy, remaining);
    }

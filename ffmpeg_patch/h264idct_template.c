...
static av_always_inline void FUNCC(cv_fill_4x4_mid)(uint8_t *_dst, int stride)
{
    int i;
    pixel *dst = (pixel *)_dst;
    stride >>= sizeof(pixel) - 1; // bytes -> pixels (matches this template’s convention)
    const pixel mid = (pixel)(1 << (BIT_DEPTH - 1));
...
static av_always_inline void FUNCC(cv_fill_8x8_mid)(uint8_t *_dst, int stride)
{
    int i;
    pixel *dst = (pixel *)_dst;
    stride >>= sizeof(pixel) - 1; // bytes -> pixels
    const pixel mid = (pixel)(1 << (BIT_DEPTH - 1));

    for (i = 0; i < 8; i++) {
        dst[0] = mid; dst[1] = mid; dst[2] = mid; dst[3] = mid;
        dst[4] = mid; dst[5] = mid; dst[6] = mid; dst[7] = mid;
        dst += stride;
    }
}
...
void FUNCC(ff_h264_idct_add)(uint8_t *_dst, int16_t *_block, int stride)
{
    ...
    for(i=0; i<4; i++){
        ...
        dst[i + 0*stride]= av_clip_pixel(128 + ((int)(z0 + z3) >> 6));
        dst[i + 1*stride]= av_clip_pixel(128 + ((int)(z1 + z2) >> 6));
        dst[i + 2*stride]= av_clip_pixel(128 + ((int)(z1 - z2) >> 6));
        dst[i + 3*stride]= av_clip_pixel(128 + ((int)(z0 - z3) >> 6));
    }
...
void FUNCC(ff_h264_idct8_add)(uint8_t *_dst, int16_t *_block, int stride){
    ...
    for( i = 0; i < 8; i++ )
    {
        ...
        dst[i + 0*stride] = av_clip_pixel( 128 + ((int)(b0 + b7) >> 6) );
        dst[i + 1*stride] = av_clip_pixel( 128 + ((int)(b2 + b5) >> 6) );
        dst[i + 2*stride] = av_clip_pixel( 128 + ((int)(b4 + b3) >> 6) );
        dst[i + 3*stride] = av_clip_pixel( 128 + ((int)(b6 + b1) >> 6) );
        dst[i + 4*stride] = av_clip_pixel( 128 + ((int)(b6 - b1) >> 6) );
        dst[i + 5*stride] = av_clip_pixel( 128 + ((int)(b4 - b3) >> 6) );
        dst[i + 6*stride] = av_clip_pixel( 128 + ((int)(b2 - b5) >> 6) );
        dst[i + 7*stride] = av_clip_pixel( 128 + ((int)(b0 - b7) >> 6) );
    }
                               ...
                               const uint8_t nnzc[5 * 8])
{
    ...
    for(i=0; i<16; i++){
        ...
        if(nnz){
            ...
            // For residual visualization mode: when nnz==0 and dc==0, IDCT add is not called.
            // Without this, the prediction already in dst leaks into the residual plane.
            if (!((dctcoef*)block)[i*16])
                FUNCC(cv_fill_4x4_mid)(dst + block_offset[i], stride);
        }
    }
}
                                    ...
                                    const uint8_t nnzc[5 * 8])
{
    ...
    for(i=0; i<16; i++){
        ...
        else                            FUNCC(cv_fill_4x4_mid)(dst + block_offset[i], stride);
    }
}
                               ...
                               const uint8_t nnzc[5 * 8])
{
    ...
    for(i=0; i<16; i+=4){
        ...
        if(nnz){
            ...
            // For residual visualization mode: if nnz==0 and dc==0, fill with 128 to avoid prediction leakage.
            if (!((dctcoef*)block)[i*16])
                FUNCC(cv_fill_8x8_mid)(dst + block_offset[i], stride);
        }
    }
}
    ...
    for(j=1; j<3; j++){
        for(i=j*16; i<j*16+4; i++){
            if(nnzc[ scan8[i] ])
                ...
                FUNCC(cv_fill_4x4_mid)(dest[j-1] + block_offset[i], stride);
        }
    }
}
    ...
    for(j=1; j<3; j++){
        for(i=j*16+4; i<j*16+8; i++){
            if(nnzc[ scan8[i+4] ])
                ...
                FUNCC(cv_fill_4x4_mid)(dest[j-1] + block_offset[i+4], stride);
        }
    }
}

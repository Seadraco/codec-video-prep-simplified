...
static av_noinline void FUNC(hl_decode_mb)(const H264Context *h, H264SliceContext *sl)
{
    ...
    if (IS_INTRA_PCM(mb_type)) {
        ...
        // For residual visualization: fill PCM blocks with neutral gray (128) instead of actual data
        // This ensures PCM blocks appear as zero residual in the output
        // Note: Removed !SIMPLE check so this applies to both simple and complex paths
        if (PIXEL_SHIFT) {
            int j;
            const uint16_t mid_val = (uint16_t)(128 << (bit_depth - 8));
            for (i = 0; i < 16; i++) {
            ...
            // 8-bit: fill with 128 for residual visualization
            for (i = 0; i < 16; i++)
                memset(dest_y + i * linesize, 128, 16);
            if (SIMPLE || !CONFIG_GRAY || !(h->flags & AV_CODEC_FLAG_GRAY)) {
                if (!h->ps.sps->chroma_format_idc) {
                    for (i = 0; i < 8; i++) {
...
static av_noinline void FUNC(hl_decode_mb_444)(const H264Context *h, H264SliceContext *sl)
{
    ...
    if (IS_INTRA_PCM(mb_type)) {
        ...
        if (PIXEL_SHIFT) {
            ...
            for (p = 0; p < plane_count; p++)
                for (i = 0; i < 16; i++)
                    memset(dest[p] + i * linesize, 128, 16);
        }
    } else {
        if (IS_INTRA(mb_type)) {
            if (sl->deblocking_filter)

...
#define HEVC_CTU_BIT_COST_MAP_MAGIC MKBETAG('H', 'C', 'T', 'U')
...
static float *hevc_sub_bit_cost_ptr(HevcCtuBitCostMap *map)
{
    return (float *)(map->costs + (size_t)map->ctb_stride * map->ctb_height);
}

static const float *hevc_sub_bit_cost_ptr_const(const HevcCtuBitCostMap *map)
{
    return (const float *)(map->costs + (size_t)map->ctb_stride * map->ctb_height);
}

static av_always_inline void hevc_subcost_add_rect(HEVCContext *s, int x0, int y0,
                                                   int w, int h, float bits);

static int hevc_cabac_stream_pos_bits(const CABACContext *c)
{
    ...
    if (c->low & 0x1FF)
        ptr--;
#endif
    return (int)(ptr - c->bytestream_start) * 8;
}

static HevcCtuBitCostMap *hevc_get_ctu_bit_cost_map(HEVCContext *s)
{
    AVBufferRef *buf;
    HevcCtuBitCostMap *map;
    size_t size;
    const int sub_width = (s->ps.sps->width + 3) >> 2;
    const int sub_height = (s->ps.sps->height + 3) >> 2;

    if (!s || !s->ref || !s->ps.sps)
        return NULL;

    size = sizeof(*map) +
           (size_t)s->ps.sps->ctb_width * s->ps.sps->ctb_height * sizeof(int32_t) +
           (size_t)sub_width * sub_height * sizeof(float);
    buf = s->ref->ctu_bit_cost_buf;

    if (!buf || buf->size != size) {
        av_buffer_unref(&s->ref->ctu_bit_cost_buf);
        buf = av_buffer_allocz(size);
        if (!buf)
            return NULL;
        s->ref->ctu_bit_cost_buf = buf;
    }

    map = (HevcCtuBitCostMap *)buf->data;
    if (map->magic != HEVC_CTU_BIT_COST_MAP_MAGIC ||
        map->version != HEVC_CTU_BIT_COST_MAP_VERSION ||
        map->ctb_width != s->ps.sps->ctb_width ||
        map->ctb_height != s->ps.sps->ctb_height ||
        map->ctb_stride != s->ps.sps->ctb_width ||
        map->log2_ctb_size != s->ps.sps->log2_ctb_size ||
        map->sub_width != sub_width ||
        map->sub_height != sub_height ||
        map->sub_stride != sub_width) {
        memset(buf->data, 0, size);
        map = (HevcCtuBitCostMap *)buf->data;
        map->magic         = HEVC_CTU_BIT_COST_MAP_MAGIC;
        map->version       = HEVC_CTU_BIT_COST_MAP_VERSION;
        map->ctb_width     = s->ps.sps->ctb_width;
        map->ctb_height    = s->ps.sps->ctb_height;
        map->ctb_stride    = s->ps.sps->ctb_width;
...
static void hevc_store_ctu_bit_cost(HEVCContext *s, int ctb_addr_rs, int cost_bits)
{
    HevcCtuBitCostMap *map = hevc_get_ctu_bit_cost_map(s);
    const int ctb_size = 1 << s->ps.sps->log2_ctb_size;
    const int ctb_x = ctb_addr_rs % s->ps.sps->ctb_width;
    const int ctb_y = ctb_addr_rs / s->ps.sps->ctb_width;
    const int x0 = ctb_x << s->ps.sps->log2_ctb_size;
    const int y0 = ctb_y << s->ps.sps->log2_ctb_size;
    const int x1 = FFMIN(s->ps.sps->width, x0 + ctb_size);
    const int y1 = FFMIN(s->ps.sps->height, y0 + ctb_size);
    int sub_x0, sub_y0, sub_x1, sub_y1;
    int x, y;
    float assigned = 0.0f;
    int cell_count;
    float remainder;

    if (!map || ctb_addr_rs < 0 || ctb_addr_rs >= map->ctb_stride * map->ctb_height)
        return;

    map->costs[ctb_addr_rs] = cost_bits;

    sub_x0 = x0 >> 2;
    sub_y0 = y0 >> 2;
    sub_x1 = (x1 + 3) >> 2;
    sub_y1 = (y1 + 3) >> 2;
    cell_count = (sub_x1 - sub_x0) * (sub_y1 - sub_y0);
    if (cell_count <= 0)
        return;

    for (y = sub_y0; y < sub_y1; y++) {
        const float *row = hevc_sub_bit_cost_ptr_const(map) + (size_t)y * map->sub_stride;
        for (x = sub_x0; x < sub_x1; x++)
            assigned += row[x];
    }

    remainder = (float)cost_bits - assigned;
    if (remainder > 1e-3f || remainder < -1e-3f)
        hevc_subcost_add_rect(s, x0, y0, x1 - x0, y1 - y0, remainder);
}

static av_always_inline int hevc_bitcost_only(const HEVCContext *s)
{
    return s->avctx->skip_idct == AVDISCARD_ALL;
}

static av_always_inline int hevc_mv_only(const HEVCContext *s)
{
    return !!(s->avctx->debug & 0x40000000);
}

static av_always_inline int hevc_skip_recon(const HEVCContext *s)
{
    return hevc_bitcost_only(s) || hevc_mv_only(s);
}
                                                   ...
                                                   int w, int h, float bits)
{
    HevcCtuBitCostMap *map = hevc_get_ctu_bit_cost_map(s);
    int sub_x0, sub_y0, sub_x1, sub_y1;
    int cells_w, cells_h, cell_count;
    int x, y;
    float add;

    if (!map || bits == 0.0f || w <= 0 || h <= 0)
        return;
    ...
    if (x0 >= s->ps.sps->width || y0 >= s->ps.sps->height)
        return;

    w = FFMIN(w, s->ps.sps->width - x0);
    h = FFMIN(h, s->ps.sps->height - y0);
    if (w <= 0 || h <= 0)
        return;
    ...
    sub_x1 = (x0 + w + 3) >> 2;
    sub_y1 = (y0 + h + 3) >> 2;
    cells_w = sub_x1 - sub_x0;
    cells_h = sub_y1 - sub_y0;
    ...
    add = bits / cell_count;
    for (y = sub_y0; y < sub_y1; y++) {
        float *row = hevc_sub_bit_cost_ptr(map) + (size_t)y * map->sub_stride;
        for (x = sub_x0; x < sub_x1; x++)
            row[x] += add;
    }
}

static av_always_inline void hevc_subcost_add_range(HEVCContext *s, int bits_before,
                                                    int x0, int y0, int w, int h)
{
    int bits_after = hevc_cabac_stream_pos_bits(&s->HEVClc->cc);
    int bits = bits_after - bits_before;

    if (bits > 0)
        hevc_subcost_add_rect(s, x0, y0, w, h, (float)bits);
}
...
{
    HEVCLocalContext *lc = s->HEVClc;
    const int skip_recon = hevc_skip_recon(s);
    const int log2_trafo_size_c = log2_trafo_size - s->ps.sps->hshift[1];
    int i;

    if (!skip_recon && lc->cu.pred_mode == MODE_INTRA) {
        int trafo_size = 1 << log2_trafo_size;
        
        // Pre-fill with 128 for residual visualization mode
        // This ensures cbf=0 regions contain 128 instead of prediction
        if (s->frame && s->frame->data[0]) {
            int stride = s->frame->linesize[0];
            uint8_t *dst = &s->frame->data[0][y0 * stride + (x0 << s->ps.sps->pixel_shift)];
            for (i = 0; i < trafo_size; i++) {
                if (s->ps.sps->pixel_shift) {
                    uint16_t *row = (uint16_t *)(dst + i * stride);
                    for (int j = 0; j < trafo_size; j++)
                        row[j] = 128 << (s->ps.sps->bit_depth - 8);
                } else {
                    memset(dst + i * stride, 128, trafo_size);
                }
            }
        }
    ...
    if (cbf_luma || cbf_cb[0] || cbf_cr[0] ||
        ...
        int cbf_chroma = cbf_cb[0] || cbf_cr[0] ||
                         ...
                         (cbf_cb[1] || cbf_cr[1]));
        const int bits_before_tu = hevc_cabac_stream_pos_bits(&lc->cc);
        ...
        hevc_subcost_add_range(s, bits_before_tu, x0, y0,
                               1 << log2_trafo_size, 1 << log2_trafo_size);

        if (s->sh.cu_chroma_qp_offset_enabled_flag && cbf_chroma &&
            !lc->cu.cu_transquant_bypass_flag  &&  !lc->tu.is_cu_chroma_qp_offset_coded) {
            const int bits_before_chroma_qp = hevc_cabac_stream_pos_bits(&lc->cc);
            ...
            hevc_subcost_add_range(s, bits_before_chroma_qp, x0, y0,
                                   1 << log2_trafo_size, 1 << log2_trafo_size);
        }
        ...
        if (cbf_luma) {
            const int bits_before_res = hevc_cabac_stream_pos_bits(&lc->cc);
            ff_hevc_hls_residual_coding(s, x0, y0, log2_trafo_size, scan_idx, 0);
            hevc_subcost_add_range(s, bits_before_res, x0, y0,
                                   1 << log2_trafo_size, 1 << log2_trafo_size);
        }
        if (s->ps.sps->chroma_format_idc && (log2_trafo_size > 2 || s->ps.sps->chroma_format_idc == 3)) {
            ...
            for (i = 0; i < (s->ps.sps->chroma_format_idc == 2 ? 2 : 1); i++) {
                ...
                if (cbf_cb[i]) {
                    ...
                    hevc_subcost_add_range(s, bits_before_res,
                                           x0, y0 + (i << log2_trafo_size_c),
                                           1 << log2_trafo_size_c, 1 << log2_trafo_size_c);
                } else
                    if (!skip_recon && lc->tu.cross_pf) {
    ...
    } else if (!skip_recon && s->ps.sps->chroma_format_idc && lc->cu.pred_mode == MODE_INTRA) {
                              ...
                              const int *base_cbf_cb, const int *base_cbf_cr)
{
    ...
    if (log2_trafo_size <= s->ps.sps->log2_max_trafo_size &&
        ...
        const int bits_before_split_tu = hevc_cabac_stream_pos_bits(&lc->cc);
        split_transform_flag = ff_hevc_split_transform_flag_decode(s, log2_trafo_size);
        hevc_subcost_add_range(s, bits_before_split_tu, x0, y0,
                               1 << log2_trafo_size, 1 << log2_trafo_size);
    } else {
    ...
    if (s->ps.sps->chroma_format_idc && (log2_trafo_size > 2 || s->ps.sps->chroma_format_idc == 3)) {
        if (trafo_depth == 0 || cbf_cb[0]) {
            const int bits_before_cbf = hevc_cabac_stream_pos_bits(&lc->cc);
            cbf_cb[0] = ff_hevc_cbf_cb_cr_decode(s, trafo_depth);
            if (s->ps.sps->chroma_format_idc == 2 && (!split_transform_flag || log2_trafo_size == 3)) {
                cbf_cb[1] = ff_hevc_cbf_cb_cr_decode(s, trafo_depth);
            }
            hevc_subcost_add_range(s, bits_before_cbf, x0, y0,
                                   1 << log2_trafo_size, 1 << log2_trafo_size);
        }
...
static void hevc_export_mvs_side_data(HEVCContext *s, AVFrame *frame)
{
    const HEVCSPS *sps;
    MvField *tab_mvf;
    int log2_min_pu_size, min_pu_size;
    int min_pu_width, min_pu_height;
    int q_w, q_h;
    int cells_per_pu;
    AVFrameSideData *sd;
    typedef struct DenseHevcMotionCell {
        int32_t l0x, l0y, l1x, l1y;
    } DenseHevcMotionCell;
    DenseHevcMotionCell *cells;
    int x_pu, y_pu;
    ...
    if (!sps || !tab_mvf || !frame)
        return;

    log2_min_pu_size = sps->log2_min_pu_size;
    min_pu_size      = 1 << log2_min_pu_size;
    min_pu_width     = sps->min_pu_width;
    min_pu_height    = sps->min_pu_height;
    q_w              = frame->width >> 2;
    q_h              = frame->height >> 2;
    cells_per_pu     = FFMAX(1, min_pu_size >> 2);
    ...
    sd = av_frame_new_side_data(frame,
                                ...
                                (size_t)q_w * q_h * sizeof(DenseHevcMotionCell));
    if (!sd)
        return;

    cells = (DenseHevcMotionCell *)sd->data;
    memset(cells, 0, (size_t)q_w * q_h * sizeof(*cells));

    for (y_pu = 0; y_pu < min_pu_height; y_pu++) {
        for (x_pu = 0; x_pu < min_pu_width; x_pu++) {
            const int mv_idx = y_pu * min_pu_width + x_pu;
            const MvField *src = &tab_mvf[mv_idx];
            const int dst_x = x_pu * min_pu_size;
            const int dst_y = y_pu * min_pu_size;
            const int qx0 = dst_x >> 2;
            const int qy0 = dst_y >> 2;
            int dy, dx;

            for (dy = 0; dy < cells_per_pu && (qy0 + dy) < q_h; dy++) {
                for (dx = 0; dx < cells_per_pu && (qx0 + dx) < q_w; dx++) {
                    DenseHevcMotionCell *dst = &cells[(qy0 + dy) * q_w + (qx0 + dx)];
                    if (src->pred_flag & PF_L0) {
                        dst->l0x = src->mv[0].x >> 2;
                        dst->l0y = src->mv[0].y >> 2;
                    }
                    if (src->pred_flag & PF_L1) {
                        dst->l1x = src->mv[1].x >> 2;
                        dst->l1y = src->mv[1].y >> 2;
                    }
                }
            }
        }
...
#define POS(c_idx, x, y)                                                              \
    ...
    const int bits_before_pu = hevc_cabac_stream_pos_bits(&lc->cc);
    int x_pu, y_pu;
    int i, j;
    ...
    hevc_subcost_add_range(s, bits_before_pu, x0, y0, nPbW, nPbH);
    ...
    if (current_mv.pred_flag == PF_L0) {
        ...
        // Fill tab_mvf for each minimum PU in this prediction unit so that
    // export_mvs can later convert them into AVMotionVector side-data.
    if (tab_mvf) {
        int log2_min_pu_size = s->ps.sps->log2_min_pu_size;
        int min_pu_size      = 1 << log2_min_pu_size;
        int x, y;

        for (y = y0; y < y0 + nPbH; y += min_pu_size) {
            for (x = x0; x < x0 + nPbW; x += min_pu_size) {
                int x_pu   = x >> log2_min_pu_size;
                int y_pu   = y >> log2_min_pu_size;
                int idx_pu = y_pu * min_pu_width + x_pu;
                tab_mvf[idx_pu] = current_mv;
            }
        }
    }
}
...
static int hls_coding_unit(HEVCContext *s, int x0, int y0, int log2_cb_size)
{
    ...
    if (s->ps.pps->transquant_bypass_enable_flag) {
        const int bits_before = hevc_cabac_stream_pos_bits(&lc->cc);
        lc->cu.cu_transquant_bypass_flag = ff_hevc_cu_transquant_bypass_flag_decode(s);
        hevc_subcost_add_range(s, bits_before, x0, y0, cb_size, cb_size);
...
static int hls_decode_entry(AVCodecContext *avctxt, void *isFilterThread)
{
    ...
    while (more_data && ctb_addr_ts < s->ps.sps->ctb_size) {
        ...
        bits_before_ctu = hevc_cabac_stream_pos_bits(&lc->cc);
        more_data = hls_coding_quadtree(s, x_ctb, y_ctb, s->ps.sps->log2_ctb_size, 0);
        if (more_data < 0) {
            s->tab_slice_address[ctb_addr_rs] = -1;
            return more_data;
        }
        hevc_store_ctu_bit_cost(s, ctb_addr_rs,
                                hevc_cabac_stream_pos_bits(&lc->cc) - bits_before_ctu);
...
static int hevc_frame_end(HEVCContext *s)
{
    ...
    if (out->ctu_bit_cost_buf) {
        av_buffer_unref(&out->frame->opaque_ref);
        out->frame->opaque_ref = av_buffer_ref(out->ctu_bit_cost_buf);
        if (!out->frame->opaque_ref)
            return AVERROR(ENOMEM);

        if (out->frame_grain) {
            av_buffer_unref(&out->frame_grain->opaque_ref);
            out->frame_grain->opaque_ref = av_buffer_ref(out->ctu_bit_cost_buf);
            if (!out->frame_grain->opaque_ref)
                return AVERROR(ENOMEM);
        }

        if (s->output_frame->data[0] && out->frame->data[0] &&
            s->output_frame->data[0] == out->frame->data[0]) {
            av_buffer_unref(&s->output_frame->opaque_ref);
            s->output_frame->opaque_ref = av_buffer_ref(out->ctu_bit_cost_buf);
            if (!s->output_frame->opaque_ref)
                return AVERROR(ENOMEM);
        }
    ...
    *gb              = nal->gb;
    ...
    case HEVC_NAL_RASL_R:
        ...
        if (s->avctx->hwaccel) {
            ...
            int total_ctbs = s->ps.sps->ctb_width * s->ps.sps->ctb_height;
            if (ctb_addr_ts >= total_ctbs) {
                             ...
                             int *got_output, AVPacket *avpkt)
{
    ...
    if (*got_output &&
            ...
            hevc_export_mvs_side_data(s, rframe);
        }
    return avpkt->size;
}
...
static int hevc_ref_frame(HEVCContext *s, HEVCFrame *dst, HEVCFrame *src)
{
    ...
    dst->ctu_bit_cost_buf = av_buffer_ref(src->ctu_bit_cost_buf);
    if (src->ctu_bit_cost_buf && !dst->ctu_bit_cost_buf)
        goto fail;

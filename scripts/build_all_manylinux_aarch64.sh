...
for tag in $PY_TAGS; do
  ...
  echo "========================================"
  if [[ "$first" == "1" ]]; then
    MANYLINUX_IMAGE="$MANYLINUX_IMAGE" PY_TAG="$tag" bash "$ROOT/scripts/build_manylinux_wheel.sh"
    first=0
  else
    MANYLINUX_IMAGE="$MANYLINUX_IMAGE" REUSE_FFMPEG=1 PY_TAG="$tag" bash "$ROOT/scripts/build_manylinux_wheel.sh"
  fi
